...
const barberSelect = $("#booking-barber");
...
function renderSlots(rows) {
    if (!dateInput || !timeSelect || !dateInput.value)
        return;
    const totalByTime = new Map();
    const barberByTime = new Map();
    const selected = selectedBarberId();
    for (const row of rows || []) {
        const time = row.start_time.slice(0, 5);
        totalByTime.set(time, (totalByTime.get(time) || 0) + 1);
        if (selected && row.barber_id === selected)
            barberByTime.set(time, (barberByTime.get(time) || 0) + 1);
    }
...
async function refreshAvailability() {
    ...
    try {
        const rows = await rpc("hm_active_bookings", { p_date: dateInput.value });
        renderSlots(rows);
...
async function submitBooking(event) {
    ...
    try {
        const accepted = await rpc("hm_create_booking", {
            payload: {
...
function startGalleryRotation() {
    ...
    const render = () => {
        ...
        if (caption)
            caption.textContent = figcaption || "Hairmagic Barbershop";
        if (progress) {
...
async function loadMedia() {
    ...
    if (!isSupabaseConfigured())
        return;
    try {
        const items = await rest("media_assets?select=id,storage_bucket,storage_path,kind,title,caption,alt_text,sort_order,published,is_hero&published=eq.true&order=sort_order.asc");
        ...
        gallery.querySelector(".gallery-slide")?.classList.add("is-active");
        startGalleryRotation();
    }
    catch {
