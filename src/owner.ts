...
type Booking = { booking_date: string; status: string };
type MediaItem = {
  id: string;
...
type Feedback = { id: string; rating: number; message?: string; submitted_day: string };
...
async function assertOwner(t: string): Promise<void> {
  const rows = await rest<{ user_id: string }[]>("owner_accounts?select=user_id&limit=1", {}, t);
  if (!rows.length) throw new Error("Akun ini bukan Owner Hairmagic.");
}
...
async function loadSummary() {
  ...
  const [bookings, feedback, media] = await Promise.all([
    rest<Booking[]>(`bookings?select=booking_date,status&booking_date=gte.${today}`, {}, t),
    rest<Feedback[]>("feedback?select=id,rating,message,submitted_day", {}, t),
    rest<MediaItem[]>("media_assets?select=id,storage_bucket,storage_path,kind,title,alt_text,caption,sort_order,published,is_hero", {}, t)
  ]);
...
async function loadMedia() {
  const t = token(); if (!t) return;
  const items = await rest<MediaItem[]>("media_assets?select=*&order=sort_order.asc", {}, t);
  ...
  list.querySelectorAll<HTMLButtonElement>("[data-delete-media]").forEach(btn => btn.addEventListener("click", async () => {
    ...
    if (path) await deleteMediaObject(bucket, path, t);
...
async function loadRatings() {
  const t = token(); if (!t) return;
  const rows = await rest<Feedback[]>("feedback?select=id,rating,message,submitted_day&order=submitted_day.desc&limit=30", {}, t);
