-- Hairmagic Website: safely expose the existing Astra September RPC contract.
-- Production baseline already contains:
--   20260919061249 hairmagic_website_foundation
--   20260919061515 consolidate_media_read_policy
-- This migration preserves the existing public signatures while keeping privileged
-- table access in a non-exposed private schema.
...
declare
  ...
  v_visit timestamptz;
begin
  if char_length(customer_name) < 2 or char_length(customer_name) > 80 then
    raise exception 'Invalid customer name';
  end if;
  if customer_phone !~ '^[0-9]{8,15}$' then
...
set search_path = ''
as $$
  select * from private.hm_active_bookings(p_date);
$$;
...
set search_path = ''
as $$
  select private.hm_create_booking(payload);
$$;
...
set search_path = ''
as $$
  select private.hm_submit_feedback(payload);
$$;
